defmodule GameOfLife do
  @moduledoc "Game of life"

  # sample initial cells
  #   [[:live, :live, :dead], [:dead, :dead, :live], [:dead, :live, :dead]]
  ...
  def number_neighbours({x, y}, cells) do
    ...
    list
      |> Enum.map(fn cell -> state_of_cell(cell, cells) end)
      |> Enum.count(&(&1 == :live))
  end
  ...
  defp calculate_row(x, cells) do
    ...
    0..size - 1
      |> Enum.to_list
      |> Enum.map(fn y -> calculate_cell_state({x, y}, cells) end)
  end

  defp calculate_cell_state(coordinates, cells) do
    next_state(state_of_cell(coordinates, cells),
      number_neighbours(coordinates, cells))
  end
